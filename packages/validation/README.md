...
>
> This is a sub-module used as a dependency by both the [Dictionary Manager - Client](../client/README.md) and [Dictionary Manager - Server](../../apps/server/README.md).
>
> If you are building an application that will interact with a Dictionary Manager Server over HTTP, or wants to validate data using a Dictionary, you likely want to import the [Dictionary Manager - Client](../client//README.md).

This package provides tools to parse and validate data based on the schemas in Dictionaries.

## Parsing Data

Parsing data involves reading string values for fields defined in a Dictionary Schema and converting the values into properly typed data. For example, if a field has `"dataType": "number"` and the provided value `"123"` this will be converted from the string value into the numeric `123`. A more complicated example would take a comma-separated array value, convert each element, and return the final array. If any values cannot be properly parsed and converted based on the schema's rules, an error is returned instead.
...
- `parseRecordValues`: Parse all fields in an [UnprocessedDataRecord](https://github.com/Pan-Canadian-Genome-Library/dictionary-manager/blob/develop/docs/important-concepts.md#datarecord-and-unprocesseddatarecord) based on a schema definition. Applies `parseFieldValue` to each field.
- `parseSchemaValues`: Parse all records in a collection belonging to an individual [schema](https://github.com/Pan-Canadian-Genome-Library/dictionary-manager/blob/develop/docs/important-concepts.md#schema). Applies `parseRecordValues` to each record.
- `parseDictionaryValues`: Parse all records for multiple schemas in a [dictionary](https://github.com/Pan-Canadian-Genome-Library/dictionary-manager/blob/develop/docs/important-concepts.md#dictionary). Applies `parseSchemaValues` to each array of records provided.
...
All validation functions return a [`TestResult`](https://github.com/Pan-Canadian-Genome-Library/dictionary-manager/blob/develop/docs/important-concepts.md#testresult) object that will indicate if the validation passed or failed. If the validation is successful then this result will simply indicate that the data is valid. If there were any errors then the response will include a error information.
